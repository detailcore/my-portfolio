...
const data: IData = {
  ...
  posts: [
    ...
    {
      id: 6,
      title: 'По канонам Laravel & Vue',
      tags: ['Учеба'],
      date: '2024-01-16',
      short_description:
        'Появилась идея сделать простенький сайт SPA на голом Vue 3 и Laravel API или даже подобие интернет магазина.',
      description: `
      Идея достаточно простая, найти макет и реализовать его с нуля, но в этот раз начать с бекенда и потом перейти к фронтенду.

      Суть в том, что бы делать на бекенде всё по канонам Laravel, а фронт оптимизировать под метрики PageSpeed Insights, стек будет примерно следующим:
          Backend:
          -- Laravel 10.x
          -- MySQL
          -- Php 8.x
          ...
          -- TypeScript (по возможности не сильно отвлекаться на типизацию)
          -- Pinia
          -- Vue Router
          -- UnoCSS
          -- Prettier & ESLint
          ...
          Other: 
          -- В этот раз всё на одном домене, и api и фронтенд.
          -- Потом сделать чат с использованием WebSocket (or Pusher)
          
      Ограничение: с нуля фронт и бэк сделать максимум за 15 дней, оптимально 10.`,
    },
  ],
  works: [
    {
